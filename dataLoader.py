...
def loadBurden(subject, modality, baseFolder, normalize=True):
    pet_path = baseFolder + "/DADES PET_loads"+"/PET_loads/"+subject+"/PET_PVC_MG/" + modality
...
def computeAvgSC_HC_Matrix(classification, baseFolder):
    ...
    sc_folder = baseFolder+"/connectomes"+'/'+HC[0]+"/DWI_processing"
    SC = np.loadtxt(sc_folder+"/connectome_weights.csv")

    sumMatrix = SC
    for subject in HC[1:]:
        print("SC + HC: {}".format(subject))
        sc_folder = baseFolder+"/connectomes"+'/'+subject+"/DWI_processing"
        SC = np.loadtxt(sc_folder+"/connectome_weights.csv")
        sumMatrix += SC
...
def loadSubjectData(subject, correcSCMatrix=True, normalizeBurden=True):
    ...
    fMRI_path = base_folder+"/DADES fMRI"+"/fMRI/"+subject+"/MNINonLinear/Results/Restingstate"
